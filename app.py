...
def get_tasks_from_report(report_name='next'):
    ...
    try:
        # For TaskWarrior, we need to use different filters based on report name
        if report_name == 'next' or report_name == 'ready':
            # Get pending tasks with ready status
            result = run_task_command(['export', 'status:pending', '+READY'])
        elif report_name == 'focus':
            # Use your custom focus report filter if it exists
            result = run_task_command(['export', 'status:pending'])
        else:
            # For other reports, try to use them as filters
            result = run_task_command(['export', 'status:pending'])
        ...
        # Filter to only pending tasks and sort by urgency
        pending_tasks = [task for task in tasks if task.get('status') == 'pending']
        # Sort by urgency (descending) 
        pending_tasks.sort(key=lambda x: x.get('urgency', 0), reverse=True)
