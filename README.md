...
> v2.5.0 · source-available, AGPL-3.0
...
### POST /task/\<id\>/tags
Adds one or more tags to a task in a single call. Body: `{"tag": "tagname"}`, or multiple at once, delimited by comma, space, and/or `+`: `{"tag": "work, home +errands"}`. A leading `+` on a token is stripped (`+work` → `work`); already-present tags are a no-op, not an error. Each `-` in a token is converted to `_` before validation (`work-project` → `work_project`, `a--b` → `a__b`) rather than dropped, since hyphen is confirmed unsupported by TaskWarrior's own `+tag` syntax but the intent is easy to preserve. Tokens with any other character outside `[A-Za-z0-9_]` (letters, digits, underscore) are still silently dropped rather than sent to TaskWarrior (an unrecognized token can make TaskWarrior's `modify` overwrite the task's description instead of failing cleanly, so invalid tags are filtered out before that's possible).
